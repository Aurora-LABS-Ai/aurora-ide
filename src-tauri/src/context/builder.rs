...
impl MessageBuilder {
    ...
    pub fn build_for_request(&self, context: &ContextManager, new_user_message: &str, ide_context: Option<&str>) -> Vec<ApiMessage> {
        ...
        let content = if let Some(ctx) = ide_context {
            format!("{}\n\n<user_query>\n{}\n</user_query>", ctx, new_user_message)
        } else {
    ...
    fn add_full_turn_messages(&self, messages: &mut Vec<ApiMessage>, turn: &Turn) {
        ...
        let user_content = if let Some(context) = &turn.user_context {
            format!("{}\n\n<user_query>\n{}\n</user_query>", context, turn.user_message)
        } else {
    ...
    fn add_round_messages(&self, messages: &mut Vec<ApiMessage>, round: &ToolCallRound) {
        ...
        if !round.tool_calls.is_empty() {
            ...
            let tool_calls: Vec<ApiToolCall> = round.tool_calls.iter()
                .map(|tc| tc.into())
                .collect();
