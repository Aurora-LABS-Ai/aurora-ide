...
impl ContextManager {
    ...
    pub fn add_user_message(&mut self, content: String, ide_context: Option<String>) -> Turn {
        ...
        let turn = Turn::new(self.thread_id.clone(), content, ide_context, turn_index);
    ...
    pub fn add_assistant_response(
        ...
        let mut round = ToolCallRound::new(current_turn.id.clone(), content, round_index);
    ...
    pub fn get_state(&self) -> ContextState {
        let mut state =
            ContextState::new(self.thread_id.clone(), self.context_window, self.max_output);
...
pub fn atomic_add_user_message(
    ...
    let manager = store
        ...
        .or_insert_with(|| ContextManager::new(thread_id.to_string(), context_window, max_output));
...
pub fn atomic_add_assistant_response(
    ...
    store
        .get_mut(thread_id)
        .and_then(|m| m.add_assistant_response(content, thinking))
}
...
pub fn atomic_add_tool_call(
    ...
    store
        .get_mut(thread_id)
        .and_then(|m| m.add_tool_call(id, name, arguments))
}
...
pub fn atomic_add_tool_result(
    ...
    store
        .get_mut(thread_id)
        .and_then(|m| m.add_tool_result(tool_call_id, content, is_error))
}
...
pub fn atomic_finalize_turn(thread_id: &str) -> Option<Turn> {
    ...
    store
        .get_mut(thread_id)
        .and_then(|m| m.finalize_current_turn())
}
...
pub fn atomic_set_turn_summary(thread_id: &str, turn_id: &str, summary: String) -> bool {
    ...
    store
        .get_mut(thread_id)
        .map(|m| m.set_turn_summary(turn_id, summary))
        .unwrap_or(false)
}
...
pub fn atomic_get_turns(thread_id: &str) -> Vec<Turn> {
    ...
    store
        .get(thread_id)
        .map(|m| m.turns().to_vec())
        .unwrap_or_default()
}

/// ATOMIC: Build messages
pub fn atomic_build_messages(
    thread_id: &str,
...
pub fn atomic_get_turn_to_summarize(thread_id: &str) -> Option<Turn> {
    ...
    store
        .get(thread_id)
        .and_then(|m| m.get_turns_to_summarize().cloned())
}
...
pub fn init_context_from_turns(
    ...
    let mut store = CONTEXT_STORE.write().unwrap();
    let manager =
        ContextManager::from_turns(thread_id.to_string(), turns, context_window, max_output);
