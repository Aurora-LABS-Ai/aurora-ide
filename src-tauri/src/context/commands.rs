...
pub async fn context_add_user_message(
    ...
    Ok(atomic_add_user_message(&thread_id, content, ide_context, context_window, max_output))
}
...
pub async fn context_add_tool_call(
    ...
    Ok(atomic_add_tool_call(&thread_id, tool_call_id, name, arguments))
}
...
pub async fn context_add_tool_result(
    ...
    Ok(atomic_add_tool_result(&thread_id, tool_call_id, content, is_error))
}

/// Finalize the current turn (ATOMIC)
#[tauri::command]
pub async fn context_finalize_turn(
    thread_id: String,
...
pub async fn context_build_messages(
    ...
    Ok(atomic_build_messages(&thread_id, system_prompt, token_budget))
}
...
pub async fn context_build_request_messages(
    ...
    atomic_add_user_message(&thread_id, new_message, ide_context, context_window, max_output);
    // Then build messages
    Ok(atomic_build_messages(&thread_id, system_prompt, token_budget))
}
...
#[tauri::command]
pub async fn context_needs_summarization(
    thread_id: String,
) -> Result<bool, String> {
...
#[tauri::command]
pub async fn context_clear_thread(
    thread_id: String,
) -> Result<(), String> {
...
pub async fn context_init_from_thread(
    ...
    Ok(init_context_from_turns(&thread_id, turns, context_window, max_output))
}

/// Get all turns for a thread (ATOMIC)
#[tauri::command]
pub async fn context_get_turns(
    thread_id: String,
        ...
        match msg {
            ...
            ApiMessage::Assistant { content, tool_calls } => {
                ...
                if let Some(calls) = tool_calls {
                    for tc in calls {
                        total_tokens += TokenService::count_tokens(&tc.function.name, EncodingType::Cl100k)
                            .map(|c| c.tokens as u32)
                            .unwrap_or(0);
                        total_tokens += TokenService::count_tokens(&tc.function.arguments, EncodingType::Cl100k)
                            .map(|c| c.tokens as u32)
                            .unwrap_or(0);
