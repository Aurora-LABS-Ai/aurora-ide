  ...
  return {
    ...
    executeCommandStream: vi.fn(async (requestId: string) => {
      ...
      if (onChunk) {
        ...
        onChunk({ payload: { stream: 'meta', data: '', done: true, exitCode: 0, success: true } });
      }
...
describe('shell_execute terminal routing', () => {
  it('writes streamed output to the initially selected terminal session', async () => {
    registerShellExecutors();

    const s1Writes: string[] = [];
    const s2Writes: string[] = [];

    useWorkspaceStore.setState({ rootPath: 'C:\\' } as any);
    ...
    const result = await tool!.executor({ command: 'echo test' }, 'tc1');
