  ...
  for (const server of servers) {
    ...
    if (server.status === 'connected' && server.config.enabled) {
      for (const tool of server.tools) {
        tools.push(mcpToolToDefinition(server.config.id, server.config.name, tool));
      }
    }
  }
...
export function getMcpToolsSummary(): string {
  ...
  // Log for debugging
  console.log('[MCP Tools] Getting summary, servers:', servers.length, servers.map(s => ({
    name: s.config.name,
    status: s.status,
  ...
  if (connectedServers.length === 0) {
    ...
    if (configuredServers.length > 0) {
      ...
      lines.push('');
      lines.push('The user needs to connect these servers from Settings > MCP Servers before you can use their tools.');
      lines.push('');
      return lines.join('\n');
    }
    return '';
  ...
  for (const server of connectedServers) {
    ...
    if (server.tools.length > 0) {
      ...
      for (const tool of server.tools) {
        const toolName = `mcp_${server.config.id.replace(/[^a-zA-Z0-9]/g, '_')}_${tool.name}`;
  ...
  lines.push('To use MCP tools, call them like any other tool with the full prefixed name.');
  lines.push('');
...
export function getToolDisplayName(toolName: string): string {
  ...
  const parsed = parseMcpToolName(toolName);
  if (parsed) {
    const displayName = `${parsed.serverName}: ${parsed.originalToolName}`;
  ...
  const knownToolPatterns = [
    'db_info', 'list_tables', 'describe_table', 'read_query', 'write_query',
    'create_record', 'read_records', 'update_record', 'delete_record',
    'query', 'execute', 'search', 'find', 'get', 'list', 'create', 'update', 'delete',
    'read', 'write', 'call', 'invoke'
  ];
  ...
  if (parts.length >= 2) {
    ...
    const verbPrefixes = ['db', 'list', 'get', 'create', 'update', 'delete', 'read', 'write', 'query', 'search', 'find', 'call', 'describe'];
...
export function mcpToolToDefinition(
  ...
  mcpToolDisplayNameCache.set(toolName, `${serverName}: ${tool.name}`);
  
  // Convert MCP input schema to FunctionParameters format
  const inputSchema = tool.inputSchema as FunctionParameters | undefined;
  const parameters: FunctionParameters = inputSchema && inputSchema.type === 'object'
    ? {
        type: 'object',
  ...
  return {
    ...
    function: {
      name: toolName,
      description: `[MCP: ${serverName}] ${tool.description || `Call ${tool.name} tool from ${serverName} MCP server`}`,
      parameters,
    },
  };
}
